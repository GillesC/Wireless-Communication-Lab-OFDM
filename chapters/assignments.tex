...
Following the \gls{ofdm} transmitter structure, find the structure of the receiver, as defined in \url{https://github.com/gnuradio/gnuradio/gr-digital/python/digital/ofdm_txrx.py}. To do this, write down all block connections (\texttt{self.connect}) and find search for the details regarding each block and function call. Examples are: \mintinline{python}{ofdm_frame_equalizer_vcvc}, \mintinline{python}{ofdm_sync_sc_cfb} and \mintinline{python}{channelest}. You can use \gls{grc} to help replicate the internal flow graph if the \gls{ofdm} receiver.